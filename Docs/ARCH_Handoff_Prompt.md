...
**The working tree has uncommitted changes** (verified correct by the prior session; review diffs, then commit):
...
- `CLAUDE.md`, `Docs/ARCH_Build_Governance.md`, `Docs/ARCH_PostLaunch_Roadmap.md`, `Docs/ARCH_Module2-4_Reading_Cards_DRAFT.md` — B3 rulings (callback swap, 25-screen count, Step-11 mapping) and the citation-authority flip.
- Three new untracked docs: the Comprehensive Report, Fix Session Prompts, and Verification Checklist (include in the docs commit).
...
## Open decisions — ASK ME before building the affected screens (report §6.4)
1. **B5, decision-point implementation** (gates 2.1.4 and 2.2.4): build as Stage 3 Template 4 (discrete branching screens; option counts fixed at ruling time — the catalog contains NO numeric counts) vs continue Module 1's absorb-into-guided-practice pattern. Prompt 7 in the fix pack has both variants.
2. Pilot AI-practice disclosure in 2.1: yes/no.
3. localStorage key scheme for M2 lesson-scoped keys (continue l7–l10 vs module-scoped).
4. Lesson 2.2 sixth-screen allocation (spec recommends a second guided practice: the Spiral Reinforcement Map).
5. Lesson 2.4 eight-screen composition, incl. how Task 2 onboarding/submit maps to M1's Task 1 pattern.
6. Show Expert Reasoning toggle × Layered Architecture Annotation Panel interplay on m2-l3-s2 (panel routes through panelManager.js as `annotationPanel` — already an allowed value).
7. Reading-card hosts (recommended split: Weinstein et al. 2018 on the 2.2 reflection; MIT Open Learning on the 2.4 reflection; snippets are ready in `Docs/ARCH_Module2-4_Reading_Cards_DRAFT.md`).
8. Provisional celebration text overriding the PathPoint deferral.
9. Retrieval warm-up two-lessons-prior convention at the module boundary.
10. Gated M1 remediation (run only when I rule; Prompts 8–9 in the fix pack): glossary tagging for competency/dependency/formative-assessment; FK sentence-splitting pass on the flagged cluster (worst: m1-l4b-s4, m1-l1b-s4b, m1-overview).
